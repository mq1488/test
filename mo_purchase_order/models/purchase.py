...
class procurement_order(models.Model):
    ...
    def make_po(self, cr, uid, ids, context=None):
        ...
        for procurement in self.browse(cr, uid, ids, context=context):
            ...
            sale_line_id_test = self.pool.get('procurement.order').browse(cr, uid, proc_id - 1,
                                                                          context=context).sale_line_id
            sale_line_id = self.pool.get('procurement.order').browse(cr, uid, proc_id - 1,
                                                                     context=context).sale_line_id.id
            ...
            if not partner:
                ...
                if sale_line_id_test.discount >= 30 and sale_line_id_test.discount < 50:
                    ...
                    line_vals['date_planned'] = line_vals['date_planned'] + dt.timedelta(days=30)
                    ...
                    available_draft_po_ids = po_obj.search(cr, uid, [
                        ...
                        ('is_discount', '=', True),
                        ('create_date', '>=',
                         '{}'.format(datetime.strptime(date_start, (DEFAULT_SERVER_DATETIME_FORMAT)))),
                        ('create_date', '<=',
                         '{}'.format(datetime.strptime(date_end, (DEFAULT_SERVER_DATETIME_FORMAT))))], context=context)
                    po_draft = 0
